...
Adding a new variable in for the GLM regression. Assuming that a high vote_average is what classifies as popular and high performing movie, then we will separate out any vote_average that is greater than or equal to 8. 
...
revenue, popularity, budget, vote_count, runtime, Adventure, Animation, Drama, Horror, Music, Science Fiction, Thriller, War and Western.
...
tmdb_glm2 = glm(formula = vote_rate ~ revenue + popularity + budget + vote_count + runtime + Animation + Drama + Horror + Music + `Science Fiction` + Thriller + Western + War + Adventure,
               data = trainingset,
               family = "gaussian"
                 )
...
Let's have a look at the second model's evaluation first:
...
glm_evaluation_measures2 <- get_evaluation_measurements2("GLM",
...
By creating a new variable, the GLM model has provided a higher accuracy and higher precision. This shows that there is a high precision in predicting which movie is classified as a high performing and popular movie.
